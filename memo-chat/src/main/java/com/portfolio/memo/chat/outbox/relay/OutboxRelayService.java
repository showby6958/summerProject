...
import com.portfolio.memo.chat.outbox.domain.publisher.RedisStreamOutboxPublisher;
...
public class OutboxRelayService {
    ...
    public int requeueStaleProcessing() {
        Instant cutOff = Instant.now().minusSeconds(props.getProcessingTimeoutSeconds());
    ...
    public void markSent(Long outboxId, String streamRecordId) {
        ...
        e.markSent(Instant.now(), streamRecordId);
    }
    ...
    public void markFailed(Long outboxId, Exception ex) {
        ...
        if (e.getAttemptCount() >= props.getMaxAttempts()) {
            e.markDead(Instant.now(), err);
            return;
        }

        Instant nextRetryAt = calcNextRetryAt(e.getAttemptCount());
        e.markPendingRetry(nextRetryAt, err);
    }

    private Instant calcNextRetryAt(int attemptCount) {
        ...
        return Instant.now().plusNanos((delay + jitter) * 1_000_000);
    }
